...
class testClientModule(unittest.TestCase):
    ...
    def test_MsgAnalysis(self):
        ...
        questionObj = MsgAnalysis(opinionRequest)
        questionObj.classifyMsg()
        # Control that the message is classified as question, opinion and activity
        self.assertTrue(Tags.question in questionObj.tags, "Message is actually a question.")
        self.assertTrue(Tags.opinion in questionObj.tags, "The message actually asks for an opinion.")
        self.assertTrue(Tags.activity in questionObj.tags, "The message contians an activity.")
        self.assertFalse(Tags.requestActivity in questionObj.tags, "The message does not request for an activity.")
        self.assertEqual(questionObj.activity, "tennis", "The activity attribute should be \"tennis\"")

        #Case: We could play some football?
        requestActivity = "\nUser: We could play some football?"
        questionObj = MsgAnalysis(requestActivity)
        questionObj.classifyMsg()
        # Control that the message is classified correct
        self.assertFalse(Tags.question in questionObj.tags, "Message should not have the question tag.")
        self.assertFalse(Tags.opinion in questionObj.tags, "Message does not ask for opinion")
        self.assertTrue(Tags.activity in questionObj.tags, "The message contians an activity")
        self.assertTrue(Tags.requestActivity in questionObj.tags, "The message does request for an activity.")
        self.assertEqual(questionObj.activity, "football", "The activity attribute should be \"football\"")
        self.assertTrue(Tags.sport in  questionObj.tags, "The message contians a sport activity.")
        
        #Case: Can we paint a painting?
        requestActivity = "\nUser: Can we paint a painting?"
        questionObj = MsgAnalysis(requestActivity)
        questionObj.classifyMsg()
        # Control that the message is classified correct
        self.assertFalse(Tags.question in questionObj.tags, "Message should not have the question tag.")
        ...
        self.assertTrue(Tags.activity in questionObj.tags, "The message contians an activity")
        self.assertTrue(Tags.requestActivity in questionObj.tags, "The message does request for an activity.")
        self.assertEqual(questionObj.activity, "painting", "The activity attribute should be \"paint\"")
        self.assertTrue(Tags.art in  questionObj.tags, "The message contians an art activity.")
